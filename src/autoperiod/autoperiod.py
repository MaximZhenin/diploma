...
class Autoperiod(object):
    def __init__(self, times, values, plotter=None, threshold_method='mc', mc_iterations=40, confidence_level=.9): #mc_iterations = 40
        ...
        self.period_idx_main = 0

        self.all_valid_periods = []
        ...
        self.set_periods = set()

        for i, p in self._period_hints:
            # add split_point
            is_valid, y_acf, period, split_point = self.validate_hint(i)
            if is_valid:
                self.all_valid_periods.append([y_acf, period, split_point])
        ...
        for i, p in self._period_hints:
            ...
            is_valid, y_acf, period, split_point = self.validate_hint(i)
            if is_valid:
                self.period_idx_main = p
                break

        self._period = None
        # add
        self._period_split = None
        #
        self._sinwave = None
        ...
        if period and is_valid:
            self._period = period
            # add
            self._period_split = split_point
            #
            ...
            self.shifting = self.times[np.argmax(self._sinwave)]
    ...
    def filter_periods(self, lst_of_tuples):
        seen = set()
        seen_add = seen.add
        return [x for x in lst_of_tuples if not (x[1] in seen or seen_add(x[1]))]


    @property
    def get_filter_valid_periods(self):
        # return self.filter_periods(self.all_valid_periods)
        all_periods = self.filter_periods(self.all_valid_periods)
        if len(all_periods) == 0:
            return []
        else:
            return [all_periods[0]] + sorted(self.filter_periods(all_periods[1:]), key=lambda x: -x[0])

    @property
    def get_all_hints(self):
        return self._period_hints

    @property
    def get_valid_periods(self):
        return self.all_valid_periods
    ...
    def validate_hint(self, period_idx):
        ...
        valid = min_slope1 > min_slope2 and not np.isclose(np.abs(angle1 - angle2), 0, atol=0.1) # 0.01
        window = self.acf[search_min:search_max + 1]
        peak_idx = np.argmax(window) + search_min

        if self.plotter and (valid or self.plotter.verbose) and (self.times[peak_idx] not in self.set_periods):
            self.set_periods.add(self.times[peak_idx])
        ...
        # add return self.times[t_split]
        return valid, self.acf[peak_idx], self.times[peak_idx], self.times[t_split]
